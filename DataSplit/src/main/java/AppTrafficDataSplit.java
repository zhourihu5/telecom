...
public class AppTrafficDataSplit {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = new Job(_conf, "AppTrafficDataSplit");
        job.setJarByClass(AppTrafficDataSplit.class);
    ...
    public static class DataSplitMap extends Mapper<Object, Text, Text, Text> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            //获取一行数据，使用\t进行分割。若分割后形成的数组大于8（角标为8的字段为日期格式数据），并且角标为8的字段不等于空。
            if (value.toString().trim().split("\\t").length>8&& value.toString().trim().split("\\t")[8]!=""){
                //截取出数据中的日期数据（含时间格式为yyyy-MM-dd HH:mm:ss）
                String dateTime = value.toString().trim().split("\\t")[8];
                //若数据中包含空格
    ...
    public static class DataSplitReduce extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            buff = datas.getBytes();
            //设置HDFS目录
            dfs = new Path("/AppTrafficDatas/" +year+"/"+month+ "/"+day+ "/"+key.toString()+".txt");
            //实例输出流
