...
<body class=" theme-blue">
					...
					<div class="row">
						<div id="control">
							<div id="control1" class="form-group">
								...
								<div class="col-sm-3">
									<input type="text" id="dateControl" value="${date}" name="date"
										class="form-control">
								</div>
							</div>
							<c:choose>
							...
							<div id="control3" class="form-group">
								<label class="col-sm-1 control-label" for="speedType"
									style="width: 9%;"> <spring:message code="appflow"></spring:message>：
								</label>
								<div class="col-sm-3" style="width: 15%;">
									<select type="text" id="speedType" name="speedType"
										class="form-control">
										<option value="ALL"><spring:message code="allflow"></spring:message></option>
							...
							<div class="col-sm-2">
								<button type="submit" type="button" class="btn btn-danger"
									onclick="appRateRank_control();appRatePie_control();">
			...
			<div class="panel panel-default">
				...
				<div id="page-stats" class="panel-collapse panel-body collapse in">
					<div class="row" id="report">
						<div id="reportLeft"
							style="height: 650px; display: inline; float: left; width: 65%;"></div>
						<div id="reportRight"
							style="height: 650px; display: inline; float: left; width: 35%;"></div>
					</div>
				</div>
			</div>
		</div>
	...
	/* 关闭父页面的遮罩层 */
	 window.parent.reqLoading(false);
		...
		// 直方图
		var appRate_historgram = echarts.init(document.getElementById('reportLeft'));
		var option_historgram = ${optionStr_historgram};
		appRate_historgram.setOption(option_historgram);
		//饼图
		var appRate_pie = echarts.init(document.getElementById('reportRight'));
		var option_pie = ${optionStr_pie};
		appRate_pie.setOption(option_pie);
		//提交筛选信息，触发JS
		function appRateRank_control(){
			 /* 调用加载层 */
		    reqLoading(true);
		    ...
		    $.ajax({
				type : "post",
				url : "<%=basePath%>appRateHistorgram_control",
				data : {"date" : dateControl,"network_name" : operator,"speed_type" : speedType},
				ContentType : "application/x-www-form-urlencoded; charset=UTF-8",
				dataType : "json",
				success : function(data) {
					var appRateHistorgram_control = echarts.init(document.getElementById('reportLeft'));
					appRateHistorgram_control.setOption(data);
				}
			});
		}
		function appRatePie_control(){
			var dateControl = $('#dateControl').val();
			var operator = $('#operator').val();
			var speedType = $('#speedType').val();
			 $.ajax({
				type : "post",
				url : "<%=basePath%>appRatePie_control",
				data : {"date" : dateControl,"network_name" : operator,"speed_type" : speedType},
				ContentType : "application/x-www-form-urlencoded; charset=UTF-8",
				dataType : "json",
				success : function(data) {
					/* 调用加载层 */
					reqLoading(false);
					var appRatePie_control = echarts.init(document.getElementById('reportRight'));
					appRatePie_control.setOption(data);
