...
<body>
	<div id="add_main">
			<div class="form-group">
				...
				<div class="col-sm-8">
					<input id="uname" type="text" class="form-control" placeholder="请输入用户名" name="uname">
				</div>
			</div>
			<div class="form-group">
				<label for="upassword" class="col-sm-4 control-label"><spring:message code="upassword"></spring:message>&nbsp;&nbsp;:</label>
				<div class="col-sm-8">
					<input id="upassword" type="password" class="form-control" placeholder="请输入密码" name="upassword">
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group" style="text-align: center;">
      			<button type="submit" id="submit" class="btn btn-info"><spring:message code="submit"></spring:message></button>
  			</div>
	</div>
	<script type="text/javascript">
	 $('#submit').on('click',function(){
		...
		var NWOperator = $('#NWOperator').val();
		 $.ajax({
				type : "post",
				url : "<%=basePath%>user_add",
				data : {"uname" : uname,"upassword" : upassword,"role" : role,"NWOperator":NWOperator},
				dataType : "json",
				async : false,
