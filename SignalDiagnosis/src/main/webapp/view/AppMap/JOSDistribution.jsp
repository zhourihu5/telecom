...
<script type="text/javascript">
 	   ...
 	   url : '<%=basePath%>showNearOS',
				...
				success : function(datas) {
					...
					$.each(datas, function(i, item) {
						//avgSpeed2[i]=item;
						 showlogo(item.tableCenterLng,item.tableCenterLat,item.osAndVersion,item.count);
					});
  ...
  function showtable(avgLng,avgLat,itemLng,itemLat){ 
   ...
   function returns(){
	   ...
	   window.location.href="<%=basePath%>view/AppMap/showJO.jsp";
		
   }

  
  function showlogo(tableCenterLng,tableCenterLat,osAndVersion,count){ 

	  

	  if (osAndVersion=="unknown"){
		 // alert(2);
	  }else{
		  ...
		  var label = new BMap.PanoramaLabel(osAndVersion, labelOptions);
