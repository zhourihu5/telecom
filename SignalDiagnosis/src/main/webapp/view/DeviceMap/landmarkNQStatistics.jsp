...
<body class="theme-blue">
						...
						<div id="control">
							...
							<div id="control3" class="form-group">
								<label class="col-sm-1 control-label" for="nwType"
									style="width: 9%;"> <spring:message code="nmtype"></spring:message>：
								</label>
								<div class="col-sm-3" style="width: 15%;">
							...
							<div class="col-sm-2">
								<button type="submit" type="button" class="btn btn-danger"
									onclick="landmarkNQStatistics_control()">
	...
	/* 关闭父页面的遮罩层 */
	 window.parent.reqLoading(false);
		...
		//直方图
		var landmarkNQStatistics = echarts.init(document.getElementById('main'));
		var option = ${optionStr};
		landmarkNQStatistics.setOption(option);
		//提交筛选信息，触发JS(直方图)
		function landmarkNQStatistics_control(){
			 /* 调用加载层 */
		    reqLoading(true);
		    ...
		    $.ajax({
				type : "post",
				url : "<%=basePath%>landmarkNQStatistics_control",
				data : {
					"daytime" : dateControl,
					"speedType" : speedType,
					"nwType" : nwType
				...
				success : function(data) {
					/* 调用加载层 */
				     reqLoading(false);
					var landmarkNQStatistics_control = echarts.init(document.getElementById('main'));
					landmarkNQStatistics_control.setOption(data);
				}
			});
		}
