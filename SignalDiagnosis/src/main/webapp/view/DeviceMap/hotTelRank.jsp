...
<body class="theme-blue">
					...
					<div class="row">
						<div id="control">
							...
							<div id="control3" class="form-group">
								<label class="col-sm-1 control-label" for="operator"
									style="width: 9%;"> <spring:message code="telTraffic"></spring:message>：
								</label>
								<div class="col-sm-3" style="width: 15%;">
									<select type="text" id="speedType" name="speedType"
										...
										<option value="DLSpeed"><spring:message code="upflow"></spring:message></option>
									</select>
								</div>
							</div>
							<div class="col-sm-2">
								<button type="submit" type="button" class="btn btn-danger"
									onclick="hotTelRank_control();hotTelRankPie_control();">
	...
	<script src="<%=basePath%>js/echarts-all.js" type="text/javascript" charset="utf-8"></script>
	...
	/* 关闭父页面的遮罩层 */
	 window.parent.reqLoading(false);
		...
		//直方图
		var hotTelRank_historgram = echarts.init(document.getElementById('reportLeft'));
		var option_historgram = ${optionStr_historgram};
		hotTelRank_historgram.setOption(option_historgram);
		//饼图
		var hotTelRank_pie = echarts.init(document.getElementById('reportRight'));
		var option_pie = ${optionStr_pie};
		hotTelRank_pie.setOption(option_pie); 
		//提交筛选信息，触发JS(直方图)
		function hotTelRank_control(){
		    ...
		    $.ajax({
				type : "post",
				url : "<%=basePath%>hotTelRankHistorgram_control",
				data : {
					"date" : dateControl,
				...
				success : function(data) {
					/* 调用加载层 */
				    reqLoading(false);
					var hotTelRank_historgram_control = echarts.init(document.getElementById('reportLeft'));
					hotTelRank_historgram_control.setOption(data);
				}
			});
		}
		//提交筛选信息，触发JS(直方图)
		function hotTelRankPie_control(){
			var dateControl = $('#dateControl').val();
			var operator = $('#operator').val();
			var speedType = $('#speedType').val();
		    $.ajax({
				type : "post",
				url : "<%=basePath%>hotTelRankPie_control",
				data : {
					"date" : dateControl,
					"network_name" : operator,
					"speed_type" : speedType
				...
				dataType : "json",
				success : function(data) {
					var hotTelRank_pie_control = echarts.init(document.getElementById('reportRight'));
					hotTelRank_pie_control.setOption(data);
				}
			});
		}
